...
export default function NFTForms() {
  ...
  useEffect(() => {
    backgroundControls.start({ backgroundColor: NFT_TYPES.find(t => t.value === category)?.color || '#FFFFFF' });
  ...
  return (
    <Tabs value={category} onValueChange={(value) => setCategory(value as NFTCategory)}>
      ...
      <div className="lg:hidden mb-8" ref={dropdownRef}>
        <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center justify-between w-full p-3 bg-transparent border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none transition-all duration-300 ease-in-out transform hover:scale-105">
            ...
            {NFT_TYPES.map((type) => (
              ...
              >
                <div className="flex items-center space-x-2 bg-transparent">
                  <div className="w-4 h-4 rounded-full bg-transparent" style={{ backgroundColor: type.color }} />
                  <span className="font-medium bg-transparent p-1">{type.label}</span>
                </div>
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
      ...
      <TabsList className="hidden lg:grid grid-cols-7 mb-8 gap-4 bg-transparent">
        {NFT_TYPES.map((type) => (
          <TabsTrigger 
            key={type.value} 
            value={type.value}
      ...
      <TabsContent value={category} className="relative overflow-hidden rounded-xl bg-transparent">
        <motion.div
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="bg-transparent bg-opacity-90 rounded-xl shadow-2xl p-6 bg-transparent"
          >
            <FormComponent />
          </motion.div>
        </AnimatePresence>
